...
const API = `${process.env.REACT_APP_API_BASE_URL}State/GetState`;
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    let filteredData = tempData.filter(
      (x) =>
        ...
        x.stateId.includes(searchValue) ||
  ...
  const handleModal = async (id) => {
    ...
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}State/GetState/${id}`
    );
    const option = {
      replace: true,
      state: {
    ...
    navigate(routeNames.CREATESTATE, option);
  };
  ...
  const deleteState = (stateId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}State/deleteState/${stateId}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATESTATE, {
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-xxl"
                              ...
                              onClick={() => handleModal(row.stateId)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteState(row.stateId)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
