...
const API = `${process.env.REACT_APP_API_BASE_URL}Designation/GetDesignation`;
...
const headCells = [
  ...
  {
    id: "designationOrderId",
    numeric: true,
    disablePadding: false,
    label: "Designation OrderID",
  },
  {
    id: "isActive",
    numeric: true,
...
export default function EnhancedTable() {
  ...
  const handleModal = async (designationId) => {
    setOpen((prevState) => !prevState);
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}Designation/GetDesignation/${designationId}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        designationData: result.data,
      },
    };
    navigate(routeNames.CREATEDESIGNATION, option);
  };
  ...
  const deleteDesignation = (designationId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}Designation/deleteDesignation/${designationId}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATEDESIGNATION, {
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        {row.designationShort}
                      </TableCell>
                      <TableCell align="center">
                        {row.designationOrderId}
                      </TableCell>
                      <TableCell align="center">
                        {row.isActive ? "Yes" : "No"}
                      </TableCell>
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-xxl"
                              ...
                              onClick={() => handleModal(row.designationId)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteDesignation(row.designationId)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
