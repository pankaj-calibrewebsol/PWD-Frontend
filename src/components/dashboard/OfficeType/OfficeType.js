...
const API = `${process.env.REACT_APP_API_BASE_URL}OfficeType/GetOfficeType`;
...
const headCells = [
  ...
  {
    id: "officeTypeNameShort",
    numeric: true,
    disablePadding: false,
    label: "Office Type Name Short",
  },
...
export default function EnhancedTable() {
  ...
  const handleModal = async (officeTypeId) => {
    setOpen((prevState) => !prevState);
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}OfficeType/GetOfficeType/${officeTypeId}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        officeTypeData: result.data,
      },
    };
    navigate(routeNames.CREATEOFFICETYPE, option);
  };
  ...
  const deleteOfficeType = (officeTypeId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}OfficeType/DeleteOfficeType/${officeTypeId}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
          ...
          .catch(() => {
            Swal.fire("OfficeType not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATEOFFICETYPE, {
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-xxl"
                              ...
                              onClick={() => handleModal(row.officeTypeId)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteOfficeType(row.officeTypeId)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
