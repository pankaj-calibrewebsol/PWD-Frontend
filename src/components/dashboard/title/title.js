...
const API = `${process.env.REACT_APP_API_BASE_URL}title/GetTitle`;
...
export default function EnhancedTable() {
  ...
  const handleModal = async (titleId) => {
    setOpen((prevState) => !prevState);
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}Title/GetTitle/${titleId}`
    );
    ...
    navigate(routeNames.CREATETITLE, option);
  };
  ...
  const DeleteTitle = (titleId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}Title/DeleteTitle/${titleId}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATETITLE, {
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.titleId)}
                      role="checkbox"
                    ...
                    > <TableCell align="left">{row.srNo}</TableCell>
                      <TableCell align="left">{row.titleName}</TableCell>
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-sm text-center"
                              ...
                              onClick={() => handleModal(row.titleId)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          <div className="delete">
                            <button
                              className="btn
                          btn-danger btn-sm"
                              onClick={() => DeleteTitle(row.titleId)}
                            >
                              <DeleteTwoToneIcon />
                            </button>
                          </div>
