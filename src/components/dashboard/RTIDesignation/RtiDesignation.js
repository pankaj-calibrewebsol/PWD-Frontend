...
const API = `${process.env.REACT_APP_API_BASE_URL}RTIDesignation/GetRTIDesignation`;
...
const inspectionResponsibles = [
  { id: 1, name: "inspectionResponsible One" },
  { id: 2, name: "inspectionResponsible Two" },
  { id: 3, name: "inspectionResponsible Three" },
  { id: 4, name: "inspectionResponsible Four" },
];
...
export default function EnhancedTable() {
  ...
  const handleModal = async (rtiDesigId) => {
    setOpen((prevState) => !prevState);
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}RTIDesignation/GetRTIDesignation/${rtiDesigId}`
    );
    const option = {
      replace: true,
      state: {
    ...
    navigate(routeNames.CREATERTIDESIGNATION, option);
  };
  ...
  const DeleteRTIDesignation = (rtiDesigId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}RTIDesignation/DeleteRTIDesignation/${rtiDesigId}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
          ...
          .catch(() => {
            Swal.fire("RTIDesignation not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATERTIDESIGNATION, {
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-primary btn-xxl"
                              ...
                              onClick={() => handleModal(row.rtiDesigId)}
                            >
                              <EditIcon />
                            </button>
                          </div>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => DeleteRTIDesignation(row.rtiDesigId)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
