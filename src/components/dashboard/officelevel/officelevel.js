...
const API = `${process.env.REACT_APP_API_BASE_URL}OfficeLevel/GetOfficeLevel`;
...
export default function EnhancedTable() {
  ...
  const handleModal = async (id) => {
    ...
    let result = await axios.get(
      `${process.env.REACT_APP_API_BASE_URL}OfficeLevel/Get/${id}`
    );
    const option = {
      replace: true,
      state: {
        mode: Modes.edit,
        officeLevelData: result.data,
      },
    };
    navigate(routeNames.CREATEOFFICELEVEL, option);
  };
  ...
  const deleteOfficelevel = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `${process.env.REACT_APP_API_BASE_URL}OfficeLevel/deleteOfficeLevel/${id}`
          )
          .then((res) => {
            Swal.fire({
              icon: "success",
          ...
          .catch(() => {
            Swal.fire("OfficeLevel not deleted.");
          });
      }
    });
  };
    ...
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        ...
        <button
          ...
          onClick={() =>
            navigate(routeNames.CREATEOFFICELEVEL, {
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{row.officeLevel}</TableCell>
                      ...
                      <TableCell>
                        <div style={{ display: "flex" }}>
                          ...
                          btn-danger btn-sm"
                            style={{ margin: 3 }}
                            onClick={() => deleteOfficelevel(row.id)}
                          >
                            <DeleteTwoToneIcon />
                          </button>
                        </div>
